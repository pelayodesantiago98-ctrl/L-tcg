...
async function pedir(ruta, { reserva = 0, binario = false, corte } = {}) {
  ...
  const horaCaducada = edad > 3600e3;
  const diaCaducado = edad > 24 * 3600e3;
  if (!horaCaducada && cuota.restanHora != null && cuota.restanHora <= reserva) throw new SinCuota(cuota);
  if (!diaCaducado && cuota.restanDia != null && cuota.restanDia <= reserva) throw new SinCuota(cuota);
