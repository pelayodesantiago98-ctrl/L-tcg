...
const estado = () => ({
  activo: false, setCode: null, tamano: 'low', hechas: 0, total: 0,
...
async function cicloPrecarga() {
  ...
  try {
    let e = estado();
    if (!e.activo || !e.setCode) return;
    const pend = pendientesDe(e.setCode).filter((id) => !enDisco(id, e.tamano));   // enDisco devuelve ruta o null
    if (!pend.length) {
      guardar({ activo: false, mensaje: `Listo: ${e.setCode} ya tiene todas sus imágenes` });
      return;
    }
    for (const id of pend) {
      ...
      e = guardar({ hechas: e.hechas + 1, total: e.total || pend.length,
                    mensaje: `${e.setCode}: ${e.hechas + 1} de ${e.total || pend.length}` });
      await new Promise((r) => setTimeout(r, PAUSA));
    }
    guardar({ activo: false, mensaje: `Terminado: ${e.setCode}` });
  } catch (err) {
...
function precargar(setCode, tamano = 'low') {
  ...
  const e = guardar({ activo: true, setCode, tamano: TAMANOS.includes(tamano) ? tamano : 'low',
...
module.exports = {
  ...
  precargar, pararPrecarga, reanudarPrecarga, estado, cuantasEnDisco, seguro,
};
