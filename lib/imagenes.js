...
/*
 * Cada cuanto se baja una imagen.
 *
 * El ritmo es el maximo que deja el techo de la hora: se gasta el cupo del
 * dia lo antes que la API lo permita, en vez de estirarlo hasta medianoche.
 * Con el plan gratuito son 85 por hora, una imagen cada 42 segundos, y el
 * cupo del dia se agota en unas doce horas.
 *
 * Lo que protege la navegacion no es ir despacio, es RESERVA_PRECARGA: la
 * precarga se para sola cuando quedan 15 peticiones de la hora o del dia, y
 * esas 15 no las toca nadie mas que las imagenes que alguien esta esperando
 * en pantalla. Aun asi, quien entre con el cupo del dia ya gastado y pida una
 * carta que no este en disco solo tiene esas 15 hasta que el dia renueve: es
 * el precio de bajar el catalogo cuanto antes, y es deliberado.
 *
 * Antes esto repartia el cupo entre 24 horas (41/h) para que a cualquier hora
 * quedase margen. Se cambio a proposito, y conviene tener claro que el
 * catalogo no acaba antes por esto: el techo son las peticiones del dia, no
 * la velocidad, y son las mismas ~985 de una forma o de la otra. Lo que
 * cambia es cuando se gastan, de golpe por la manana en vez de goteando, y
 * que el dia cierra a cero en vez de rozar el limite.
 *
 * El suelo de PAUSA sigue valiendo: por generoso que fuese el plan, no se
 * ametralla la API. Y el ritmo sale de lo que la propia API declara, no de un
 * numero escrito aqui, para que siga valiendo si cambia el plan.
 */
function cadencia() {
  const c = api.cuotaActual();
  const hora = Math.max((c.limiteHora || 100) - api.RESERVA_PRECARGA, 1);
  return Math.max(Math.round(3600e3 / hora), PAUSA);
}
...
async function cicloPrecarga() {
  ...
  try {
    ...
    for (const id of pend) {
      ...
      /* En el catalogo el «de N» que importa es lo que queda por bajar, no lo
         hecho en esta sesion: al parar y relanzar, `hechas` vuelve a cero y
         parecia que se habia perdido todo lo anterior. */
      const restantes = e.ambito === 'catalogo' ? cuantasPendientesCatalogo() - 1 : null;
      e = guardar({ hechas: e.hechas + 1, total: e.total || pend.length,
                    mensaje: restantes === null
                      ? `${rotulo(e)}: ${e.hechas + 1} de ${e.total || pend.length}`
                      : `Catálogo: ${cuantasEnDisco() + 1} en disco, quedan ${restantes}` });
      await espera(cadencia());
    }
 ...
 * Red de seguridad.
 *
 * Reengancharse tras quedarse sin cuota o tras un reinicio ya funciona, pero
 * las dos cosas dependen de que haya un temporizador vivo. Si ese temporizador
 * se pierde -- una excepcion en mal momento, dos ciclos pisandose --, el
 * estado se queda en «activo» y nadie lo despierta: la pantalla dice que esta
 * bajando imagenes y no baja ninguna.
 *
 * Esto no intenta adivinar por que se ha perdido. Mira lo unico que importa:
 * si dice estar activa y lleva un buen rato sin dar señales de vida, se la
 * vuelve a poner en marcha. Sin cuota se guarda estado cada diez minutos, asi
 * que veinte sin una sola señal es que algo va mal de verdad.
 */
const SILENCIO_MAXIMO = 20 * 60 * 1000;
const CADA_CUANTO_MIRO = 15 * 60 * 1000;

function vigilar() {
  const reloj = setInterval(() => {
    const e = estado();
    if (!e.activo || corriendo) return;

    const desde = Date.parse(e.ultimo || '') || 0;
    if (!desde || Date.now() - desde < SILENCIO_MAXIMO) return;

    const minutos = Math.round((Date.now() - desde) / 60000);
    console.warn(`[precarga] activa pero callada desde hace ${minutos} min; la reengancho`);
    reprogramar(1000);
  }, CADA_CUANTO_MIRO);
