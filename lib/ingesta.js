  ...
  try {
    ...
    while (e.activo && e.indiceSet < pendientes.length) {
      ...
      /* La búsqueda no ha dado nada de esta expansión: se reintenta por el
         ...
         Salvo que ya estemos en ese modo: el listado no trae precios, así que
         bajarlo aquí sería pagar peticiones por lo único que no se ha venido a
         buscar. En ese caso se pasa a la siguiente expansión. */
      if (!porListado && e.pagina === 1 && delBuscado === 0) {
        const soloPrecios = e.modo === 'precios';
        e = guardar({
          via: soloPrecios ? 'busqueda' : 'listado',
          indiceSet: soloPrecios ? e.indiceSet + 1 : e.indiceSet,
          pagina: 1, peticiones: e.peticiones + 1,
          cartasGuardadas: e.cartasGuardadas + guardadas,
          conPrecio: e.conPrecio + conPrecio,
          mensaje: exp.nombre + (soloPrecios
            ? ': la búsqueda no da nada suyo, no hay precios que traer'
            : ': la búsqueda no da nada suyo, se baja por listado'),
        });
